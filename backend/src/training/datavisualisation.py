...
class AdultIncomeDataVisualisation:
    def __init__(self, dataToVisOrigin, dataToVis, config=None):
        ...
        logger.info("DataVisualisation initialized")
    ...
    def compare_datasets(self, df_orig, df_new):
        ...
        dtype_changes = {
            col: (str(df_orig[col].dtype), str(df_new[col].dtype))
            for col in orig_columns.intersection(new_columns)
            if df_orig[col].dtype != df_new[col].dtype
        }
        ...
        categorical_cols = df_orig.select_dtypes(
            include=["object"]
        ).columns.intersection(df_new.select_dtypes(include=["object"]).columns)
        category_changes = {}

        for col in categorical_cols:
            orig_categories = set(df_orig[col].dropna().unique())
            new_categories = set(df_new[col].dropna().unique())
            category_changes[col] = {
                "original": list(orig_categories),
                "neu": list(new_categories),
            }
        report["kategorische_abweichungen"] = category_changes

        logger.info("Dataset comparison done.")
        return report

    # Funktion Verteilungsanalyse der unabhängigen Variablen
    def generate_side_by_side_plots(self, df_orig, df_new, col, prefix):
        logger.info(f"Generating side-by-side plots for column '{col}'...")
        if col == "income":
            return None
        ...
        plt.close()
        logger.info(f"Side-by-side plots for column '{col}' saved.")
        return plot_path
    ...
    def plot_income_distribution(self, df_orig, df_new):
        ...
        sns.countplot(
            x="income", data=df_orig, ax=axes[0], hue="income", palette="Set1"
        )
        ...
        plt.savefig(f"{self.artifact_dir}/{plot_path}")
        plt.close()
        logger.info("Income distribution plot saved.")
        return plot_path

    def plot_new_variables(self, df_new, new_columns):
        logger.info("Plotting new variables...")
        new_plots = []
        ...
        logger.info("New variable plots saved.")
        return new_plots
    ...
    def plot_deleted_variables(self, df_orig, deleted_columns):
        logger.info("Plotting deleted variables...")
        deleted_plots = []
        for col in deleted_columns:
            ...
            if df_orig[col].dtype == "object":
                sns.countplot(
                    ...
                    data=df_orig,
                    hue="income",
                    order=df_orig[col].value_counts().index,
                )
                plt.xticks(rotation=45, ha="right")
            else:
        ...
        logger.info("Deleted variable plots saved.")
    ...
    def analyze_dataset(
        ...
        report_s["Allgemeine Datenbeschreibung"] = (
            f"Der Datensatz hat {num_rows} Zeilen und {num_cols} Spalten.\n"
        ...
        report_s["Duplikate"] = (
            f"Es gibt {duplicate_count} Duplikate ({duplicate_ratio:.2f}%). - Da weniger als 0,5% der Daten Duplikate sind, gehen wir davon aus, dass es wahre Beobachtungen sind und keine Bereinigung notwendig ist."
            if duplicate_ratio < 0.5
            else f"Achtung! Es gibt {duplicate_count} Duplikate ({duplicate_ratio:.2f}%). Eine Bereinigung könnte notwendig sein."
        )
        ...
        def eta_squared(num_col, cat_col):
            ...
            ss_between = sum(
                len(group) * (np.mean(group) - np.mean(num_col)) ** 2
                for group in groups
            )
        ...
        for col1 in cols:
            for col2 in cols:
                ...
                elif df[col1].dtype in ["int64", "float64"] and df[col2].dtype in [
                    "int64",
                    "float64",
                ]:
                    corr_matrix.loc[col1, col2] = pearsonr(
        ...
        if target_col in corr_matrix.columns:
            ...
            report_s["Top 3 Variablen mit hoechster Korrelation zur Zielvariable"] = (
                target_correlation.head(3).to_string()
            )
            report_s["Top 3 Variablen mit geringster Korrelation zur Zielvariable"] = (
                target_correlation.tail(3).to_string()
            )
        ...
        report_s["Variablen mit hoher gegenseitiger Korrelation (>0,7)"] = "\n".join(
            [
                f" - {var1} & {var2} (Korrelation: {corr:.2f}) - ggfs. nur eine Variable fuer das Model relevant"
        ...
        for col in df.columns:
            most_common_ratio = (
                df[col].value_counts(normalize=True, dropna=True).max() * 100
            )  # Anteil der häufigsten Kategorie
            if most_common_ratio > imbalance_threshold:
                skewed_vars.append((col, most_common_ratio))

        if skewed_vars:
            report_s["Ungleich verteilte Variablen"] = (
                "Folgende Variablen sind stark unausgewogen (>80% in einer Kategorie):\n"
            )
            for var, ratio in skewed_vars:
                report_s[
                    "Ungleich verteilte Variablen"
                ] += f" - {var}: {ratio:.2f}% der Werte entfallen auf eine einzige Kategorie\n. Eine Transformation koennte notwendig sein."
        else:
            report_s["Ungleich verteilte Variablen"] = (
                "Keine extrem ungleich verteilten Variablen gefunden."
            )
        ...
        for col in df.columns:
            ...
            total_missing = (
                num_missing_nan + num_missing_question
            )  # Gesamte fehlende Werte
        ...
        if missing_values:
            report_s["Fehlende Werte"] = (
                "Folgende Spalten enthalten fehlende Werte (NaN oder '?'):\n"
            )
            for var, (
                total,
                ratio,
                ...
                report_s[
                    "Fehlende Werte"
                ] += f" - {var}: {total} fehlende Werte ({ratio:.2f}%)\n"
                report_s[
                    "Fehlende Werte"
                ] += f"   - NaN-Werte: {nan_count}, '?' Werte: {question_count}\n"
                if ratio < 5:
                    report_s[
                        "Fehlende Werte"
                    ] += "   - Empfehlung: Modus oder Mittelwert verwenden\n"
                else:
                    report_s[
                        "Fehlende Werte"
                    ] += "   - Empfehlung: Imputation oder Entfernen pruefen\n"
        else:
            report_s["Fehlende Werte"] = (
                "Keine fehlenden Werte (NaN oder '?') gefunden."
            )
        ...
        if target_col in df.columns:
            ...
            if majority_ratio > imbalance_threshold:
                report_s["Zielvariable-Verteilung"] = (
                    f"Achtung! Die Zielvariable '{target_col}' ist unausgeglichen.\n"
                ...
                report_s["Zielvariable-Verteilung"] = (
                    f"Die Zielvariable '{target_col}' ist relativ ausgewogen verteilt."
                )
        else:
            report_s["Zielvariable-Verteilung"] = (
                "Zielvariable 'income' nicht im Datensatz gefunden."
            )
    ...
    def compute_correlation_matrix(self, df):
        ...
        for i, col1 in enumerate(cols):
            for j, col2 in enumerate(
                cols[i:], i
            ):  # Berechnet nur die obere Hälfte der Matrix
                if col1 == col2:
                    corr_matrix.loc[col1, col2] = 1.0
                elif df[col1].dtype in ["int64", "float64"] and df[col2].dtype in [
                    "int64",
                    ...
                    corr_matrix.loc[col1, col2] = corr_matrix.loc[col2, col1] = (
                        pearsonr(df[col1].dropna(), df[col2].dropna())[0]
                    )
                elif df[col1].dtype == "object" and df[col2].dtype == "object":
                    contingency = pd.crosstab(df[col1], df[col2])
    ...
    def createPlots(self):
        ...
        income_plot = self.plot_income_distribution(
            self.dataToVisOrigin, self.dataToVis
        )
        comparison_plots = [
            self.generate_side_by_side_plots(
                self.dataToVisOrigin, self.dataToVis, col, "comparison"
            )
            for col in self.dataToVisOrigin.columns.intersection(
                self.dataToVisOrigin.columns
            )
            if col != "income"
        ]
        comparison_plots = [p for p in comparison_plots if p]

        new_variable_plots = self.plot_new_variables(
            self.dataToVis, report["neue_spalten"]
        )
        deleted_variable_plots = self.plot_deleted_variables(
            self.dataToVisOrigin, report["fehlende_spalten"]
        )
        ...
        report_df = pd.DataFrame(
            list(final_analysis_report.items()), columns=["Kategorie", "Ergebnisse"]
        )
        ...
        html_content = template.render(
            ...
            new_variable_plots=new_variable_plots,
            deleted_variable_plots=deleted_variable_plots,
            correlation_plot=correlation_plot_fileName,
            report_s=report_df.to_dict(orient="records"),
            report=report,
        )
        ...
        logger.info("Data visualisation completed.")
